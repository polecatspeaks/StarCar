...
| Spec-rung artifact (`docs/templates/spec-doc.md`) | **The first time a spec is written** - not before | Build it the way the design artifact was built: from the wreckage of the first real spec, as form PLUS worked exemplar. Carry the SHAPE from `design-doc.md` (instrument check, constraints before mechanism, premises declared); do not invent the CONTENT in advance. Building it now would be inventing prior art we do not have, which is the exact failure that cost the design rung four REJECT rounds. |
| Plan-rung artifact (`docs/templates/plan-doc.md`) | **The first time a plan is written** - not before | Same treatment. Note this rung already has a scar ported in from the ancestor shop - one plan shipped snippets calling constructor parameters that did not exist, three REJECT rounds - which is the missing-workflow class at the plan rung. We inherited the wound without the stitches; expect it to fire here too unless the artifact exists before the first plan. |
